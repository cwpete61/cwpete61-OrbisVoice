...
function HelpContent() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "bot",
      text: "Hi! I'm the OrbisVoice assistant. Ask me anything about the platform, billing, voice agents, referrals, or payouts.",
    },
  ]);
  ...
  async function fetchFaqs() {
    try {
      const { data } = await apiFetch<any[]>("/help/faq");
  ...
  async function sendMessage() {
    ...
    try {
      const { data } = await apiFetch<{ answer?: string; sources?: any[] }>("/help/chat", {
        method: "POST",
      ...
      setMessages((prev) => [
        ...
        {
          role: "bot",
          text: data?.data?.answer ?? "I couldn't find an answer to that. Try rephrasing.",
  ...
  async function sendFeedback(id: string, helpful: boolean) {
    try {
      await apiFetch(`/help/faq/${id}/feedback`, {
        method: "POST",
  ...
  const filteredFaqs = faqs.filter(
    (f) => !searchQ || f.question.toLowerCase().includes(searchQ.toLowerCase())
  );
        ...
        <div className="grid gap-8 lg:grid-cols-5">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="relative">
              <svg
                className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[rgba(240,244,250,0.3)]"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
                viewBox="0 0 24 24"
            ...
            {categories.length > 0 ? (
              categories.map((cat) => (
                <div key={cat}>
                  <h2 className="mb-3 text-xs font-bold uppercase tracking-widest text-[rgba(240,244,250,0.4)]">
                    {cat}
                  </h2>
                      ...
                      .map((f) => (
                        <div key={f.id}>
                          ...
                          >
                            ...
                            <svg
                              className={`h-4 w-4 shrink-0 text-[rgba(240,244,250,0.4)] transition-transform ${openFaq === f.id ? "rotate-180" : ""}`}
                              fill="none"
                              stroke="currentColor"
                              strokeWidth="2"
                              viewBox="0 0 24 24"
                          ...
                          {openFaq === f.id && (
                            <div className="px-6 pb-5">
                              <p className="text-sm text-[rgba(240,244,250,0.7)] leading-relaxed">
                                {f.answer}
                              </p>
                              <div className="mt-4 flex items-center gap-3">
                                <span className="text-xs text-[rgba(240,244,250,0.3)]">
                                  Was this helpful?
                                </span>
                                <button
                                  onClick={() => sendFeedback(f.id, true)}
                                  className="text-xs text-green-400 hover:underline"
                                >
                                  👍 Yes
                                </button>
                                <button
                                  onClick={() => sendFeedback(f.id, false)}
              ...
              <div className="text-center py-12 text-[rgba(240,244,250,0.3)] text-sm">
                {searchQ
                  ? `No FAQ entries matching "${searchQ}"`
                  : "No FAQ entries yet. Try asking the bot!"}
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col">
            <div
              className="rounded-2xl border border-white/[0.07] bg-[#0c111d] flex flex-col"
              style={{ height: "620px" }}
            >
              {/* Header */}
              <div className="flex items-center gap-3 px-5 py-4 border-b border-white/[0.05]">
                <div className="h-8 w-8 rounded-full bg-[#14b8a6]/20 flex items-center justify-center text-[#14b8a6] text-sm font-bold">
                  AI
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#f0f4fa]">OrbisVoice Assistant</p>
                  <p className="text-[10px] text-[rgba(240,244,250,0.4)]">
                      ...
                      {m.sources && m.sources.length > 0 && (
                        <div className="mt-2 pt-2 border-t border-white/20">
                          ...
                          {m.sources.slice(0, 2).map((s: any, si: number) => (
                            <p key={si} className="text-[10px] text-[rgba(240,244,250,0.4)]">
                    ...
                    <div className="bg-white/[0.05] rounded-2xl rounded-bl-sm px-4 py-3">
                      <div className="flex gap-1">
                        {[0, 1, 2].map((i) => (
                          <span
                            key={i}
                            className="h-1.5 w-1.5 rounded-full bg-[#14b8a6] animate-bounce"
