...
export default function TenantSettingsPanel({ tenantId }: { tenantId: string }) {
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE}/admin/subscribers/${tenantId}/settings`, {
    ...
    useEffect(() => {
        setKeysLoading(true);
        const token = localStorage.getItem("token");
        fetch(`${API_BASE}/admin/subscribers/${tenantId}/api-keys`, {
            headers: { Authorization: `Bearer ${token}` },
        })
            .then((r) => r.json())
            .then((d) => {
    ...
    const saveTenantSettings = async (type: "twilio" | "google") => {
        ...
        try {
            const r = await fetch(`${API_BASE}/admin/subscribers/${tenantId}/settings`, {
                method: "POST",
                headers: {
    ...
    const deleteTenantSettings = async (type: "twilio" | "google") => {
        ...
        const token = localStorage.getItem("token");
        await fetch(`${API_BASE}/admin/subscribers/${tenantId}/settings/${type}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    const createApiKey = async () => {
        ...
        setCreatingKey(true);
        const token = localStorage.getItem("token");
        const r = await fetch(`${API_BASE}/admin/subscribers/${tenantId}/api-keys`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json"
    ...
    const revokeApiKey = async (keyId: string) => {
        ...
        const token = localStorage.getItem("token");
        await fetch(`${API_BASE}/admin/subscribers/${tenantId}/api-keys/${keyId}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` }
        });
            ...
            <div className="space-y-6">
                ...
                <div className="rounded-xl border border-white/[0.04] bg-[#0c111d]/50 p-5">
                    ...
                    {settingsLoading ? (
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="block text-xs text-[rgba(240,244,250,0.6)] mb-1">Account SID</label>
                            ...
                            <div>
                                <label className="block text-xs text-[rgba(240,244,250,0.6)] mb-1">Auth Token</label>
                                <PasswordInput value={twilioConfig.authToken} onChange={(e: any) => setTwilioConfig({ ...twilioConfig, authToken: e.target.value })}
                                    className="w-full rounded border border-white/[0.08] bg-[#05080f] px-3 py-1.5 text-xs text-[#f0f4fa] outline-none focus:border-[#14b8a6]/50 transition" />
                            </div>
                            <div>
                                <label className="block text-xs text-[rgba(240,244,250,0.6)] mb-1">Phone Number</label>
                                <input type="text" value={twilioConfig.phoneNumber} onChange={(e) => setTwilioConfig({ ...twilioConfig, phoneNumber: e.target.value })}
                                    className="w-full rounded border border-white/[0.08] bg-[#05080f] px-3 py-1.5 text-xs text-[#f0f4fa] outline-none focus:border-[#14b8a6]/50 transition" />
                            </div>
                            <button onClick={() => saveTenantSettings("twilio")} disabled={settingsSaving} className="btn-primary w-full py-1.5 text-xs">
                ...
                <div className="rounded-xl border border-white/[0.04] bg-[#0c111d]/50 p-5">
                    ...
                    {settingsLoading ? (
                        ...
                        <div className="space-y-3">
                            <div>
                                <label className="block text-xs text-[rgba(240,244,250,0.6)] mb-1">Google Client ID</label>
                            ...
                            <div>
                                <label className="block text-xs text-[rgba(240,244,250,0.6)] mb-1">Google Client Secret</label>
                            ...
                            <div>
                                <label className="block text-xs text-[rgba(240,244,250,0.6)] mb-1">Gemini API Key</label>
                            ...
                            <button onClick={() => saveTenantSettings("google")} disabled={settingsSaving} className="btn-primary w-full py-1.5 text-xs">
            ...
            <div className="space-y-6">
                <div className="rounded-xl border border-white/[0.04] bg-[#0c111d]/50 p-5">
                    ...
                    <div className="flex gap-2 mb-4">
                        ...
                        <button onClick={createApiKey} disabled={creatingKey || !newKeyName.trim()} className="btn-primary px-4 text-xs whitespace-nowrap">
                            Generate
                        </button>
                    </div>
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-b border-white/[0.05] text-left">
                                    <th className="px-3 py-2 text-[10px] font-bold text-[rgba(240,244,250,0.35)]">Name</th>
                                    <th className="px-3 py-2 text-[10px] font-bold text-[rgba(240,244,250,0.35)]">Key</th>
                                    <th className="px-3 py-2 text-[10px] font-bold text-[rgba(240,244,250,0.35)] text-right">Action</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-white/[0.03]">
                                {keysLoading ? (
                                    <tr><td colSpan={3} className="px-3 py-6 text-center text-[rgba(240,244,250,0.4)]">Loading...</td></tr>
                                ) : apiKeys.length === 0 ? (
                                    <tr><td colSpan={3} className="px-3 py-6 text-center text-[rgba(240,244,250,0.4)]">No keys</td></tr>
                                ) : apiKeys.map((k: any) => (
                                    <tr key={k.id} className="hover:bg-white/[0.01]">
                                        ...
                                        <td className="px-3 py-2 text-right">
                                            <button onClick={() => revokeApiKey(k.id)} className="text-red-400/70 hover:text-red-400 transition underline underline-offset-4">Revoke</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
