...
export default function DashboardShell({ children, tokenLoaded = true }: { children: React.ReactNode; tokenLoaded?: boolean }) {
  ...
  return (
    <div className="flex h-screen bg-[#05080f] text-[#f0f4fa] overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-[130] w-[260px] flex-col border-r border-white/[0.06] bg-[#080c16] transition-transform duration-300 ease-in-out lg:static lg:flex lg:translate-x-0 ${
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-6">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden text-[rgba(240,244,250,0.5)]">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6 custom-scrollbar">
          {["Main", "Partnership", "Admin", "System"].map((cat) => {
            const items = (NAV as any[]).filter((item) => {
              ...
              const isAffiliateOnly = typeof profile?.isAffiliate === 'boolean' ? (profile.isAffiliate && !profile.isAdmin) : false;
                  ...
                  const active = exactMatchOnly.includes(item.href)
                    ...
                    return (
                      <Link
                        ...
                        href={item.href}
                        className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition duration-300 ${active
                          ? "nav-active-glow"
        ...
        <header className="flex h-16 items-center justify-between border-b border-white/[0.05] bg-[#080c16] px-4 md:px-8">
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div className="relative" ref={notifRef}>
              <button onClick={toggleNotifs} className="relative p-2 rounded-xl border border-white/[0.05] bg-white/[0.02] hover:bg-white/[0.06] transition group focus:outline-none">
                <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className={`transition ${showNotifs ? "text-white" : "text-[rgba(240,244,250,0.5)] group-hover:text-white"}`}>
              ...
              {showNotifs && (
                <div className="absolute right-0 top-full mt-4 w-[calc(100vw-32px)] sm:w-96 rounded-2xl border border-white/[0.1] bg-[#111827] shadow-2xl z-[150] overflow-hidden transform origin-top-right transition-all animate-in fade-in slide-in-from-top-2">
                  <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06] bg-[#1a2333]">
                    <h3 className="font-bold text-[#f0f4fa] text-sm tracking-tight">Activity Center</h3>
                    <Link onClick={() => setShowNotifs(false)} href="/settings?tab=notifications" className="text-[rgba(240,244,250,0.5)] hover:text-white transition p-1.5 rounded-lg hover:bg-white/[0.05]">
                    ...
                    {notifs.length === 0 ? (
                      <div className="px-6 py-16 text-center">
                        <div className="mx-auto w-12 h-12 rounded-full bg-white/[0.03] flex items-center justify-center mb-4">
                          <svg width="24" height="24" fill="none" stroke="rgba(240,244,250,0.2)" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        </div>
                        <p className="text-sm font-medium text-[rgba(240,244,250,0.4)]">All caught up here!</p>
                      </div>
                    ) : (
                      <div className="divide-y divide-white/[0.04]">
                        {notifs.map((n: any) => {
                          ...
                          return (
                            <div key={n.id} className="flex gap-4 px-5 py-4 hover:bg-white/[0.03] transition text-left group">
                              <div className="text-2xl mt-0.5 shrink-0 grayscale group-hover:grayscale-0 transition duration-300">{meta.emoji}</div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                  <p className="text-xs font-bold text-[#f0f4fa] uppercase tracking-wider">{meta.label}</p>
                                  <p className="text-[10px] text-[rgba(240,244,250,0.3)]">
                                    {new Date(n.createdAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                                  </p>
                                </div>
                                <p className="text-xs font-semibold text-[#f0f4fa] leading-tight mb-1">{n.title}</p>
                                <p className="text-xs text-[rgba(240,244,250,0.5)] line-clamp-2 leading-relaxed">{n.body}</p>
                              </div>
                            </div>
                          );
                        })}
                  ...
                  <div className="border-t border-white/[0.06] bg-[#1a2333] p-3">
                    <Link onClick={() => setShowNotifs(false)} href="/notifications" className="text-xs font-bold text-center text-[#14b8a6] hover:text-[#2dd4bf] transition block py-2.5 rounded-xl border border-[#14b8a6]/10 hover:border-[#14b8a6]/30 bg-[#14b8a6]/5">
                      Explore Full Feed
                    </Link>
                  </div>
                </div>
