...
function NotificationsContent() {
  ...
  const fetchNotifications = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await apiFetch<{ notifications?: any[]; unreadCount?: number }>(
        `/notifications?limit=50&unreadOnly=${filter === "unread"}`
      );
      if (data?.data) {
              ...
              {(["all", "unread"] as const).map((f) => (
                <button
                  ...
                  className={`px-4 py-2 text-sm font-medium transition ${
                    filter === f
                      ? "bg-[#14b8a6] text-[#05080f]"
                      : "text-[rgba(240,244,250,0.5)] hover:text-white"
                  }`}
                >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[rgba(240,244,250,0.5)] mt-0.5 leading-relaxed">
                        {n.body}
                      </p>
                    </div>
