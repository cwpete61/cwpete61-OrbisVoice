...
function DashboardContent() {
  ...
  const fetchSubscription = async () => {
    try {
      const { data } = await apiFetch<any>("/billing/subscription");
      if (data?.data) {
      ...
      <div className="px-8 py-8">
        ...
        {subscription && (
          <section className="mb-8 rounded-2xl border border-[#14b8a6]/20 bg-[#0c111d] p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-xs text-[rgba(240,244,250,0.45)] uppercase tracking-wide">
                  Current Plan
                </p>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/[0.1] py-24 text-center">
            ...
            <p className="mt-1 text-xs text-[rgba(240,244,250,0.4)]">
              Create your first voice agent to get started
            </p>
            <Link href="/agents/new" className="btn-primary mt-6 text-sm flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-6">
            {agents.map((agent: any) => (
              <div
                key={agent.id}
                className={`group rounded-2xl border bg-[#0c111d] p-4 transition flex flex-col ${
                  agent.isActive
              ...
              >
                ...
                <div className="mb-4 flex items-start justify-between gap-3">
                  <div className="flex flex-col gap-1">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.03] text-[rgba(240,244,250,0.5)] overflow-hidden">
                      {agent.avatarUrl ? (
                        <img
                          src={agent.avatarUrl}
                          alt={agent.name}
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-[#14b8a6] uppercase tracking-tighter opacity-70">
                        {agent.creator?.username || "System"}
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-3">
                    <button
                      ...
                      className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full focus:outline-none focus:ring-2 focus:ring-[#14b8a6] focus:ring-offset-2 focus:ring-offset-[#0c111d] transition-colors ${
                        agent.isActive ? "bg-[#14b8a6]" : "bg-gray-600"
                      }`}
                    >
                      <span className="sr-only">Toggle agent active status</span>
                      <span
                        className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          agent.isActive ? "translate-x-2" : "-translate-x-2"
                        }`}
                      />
                    </button>
                ...
                <div className="flex-1 mt-3">
                  <h4
                    className={`mb-1 font-bold text-base leading-tight transition ${
                      agent.isActive
                        ? "text-white"
                        : "text-white/60"
                    }`}
                  ...
                  <p className="mb-4 line-clamp-2 text-[11px] leading-snug text-[rgba(240,244,250,0.45)]">
                    {agent.systemPrompt || <span className="italic opacity-50">No persona configured</span>}
                  </p>
                </div>

                <div className="flex items-center justify-between border-t border-white/[0.05] mt-auto pt-3 text-[10px]">
                  <span className="text-[rgba(240,244,250,0.25)] shrink-0">
                    {new Date(agent.createdAt).toLocaleDateString()}
                  </span>
                  <div className="flex items-center gap-2 font-bold ml-2 overflow-hidden">
                    <Link
                      href={`/agents/${agent.id}/conversations`}
                      className="text-[#14b8a6] hover:text-[#0d9488] transition py-1"
                    >
                      Logs
                    </Link>
                    <Link
                      href={`/agents/${agent.id}/edit`}
                      className="text-[rgba(240,244,250,0.4)] hover:text-[#f0f4fa] transition py-1"
                    >
                      Edit
                    </Link>
                    <button
                      onClick={() => handleDeleteAgent(agent.id)}
                      className="text-red-400/80 hover:text-red-300 transition py-1"
                    >
                      Delete
                    </button>
                  </div>
