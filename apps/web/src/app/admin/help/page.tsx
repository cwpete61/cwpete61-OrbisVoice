...
import { apiFetch, HelpQuestion, FaqEntry } from "@/lib/api";
...
function AdminHelpContent() {
  ...
  const [promoteForm, setPromoteForm] = useState<{
    [id: string]: { answer: string; category: string };
  }>({});
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      const { data } = await apiFetch<HelpQuestion[]>("/admin/help/questions");
      if (data?.data) {
  ...
  const fetchFaqs = useCallback(async () => {
    try {
      const { data } = await apiFetch<FaqEntry[]>("/admin/help/faq");
      if (data?.data) {
        setFaqs(data.data);
      }
    } catch (err) {
  ...
  const promoteQuestion = async (id: string) => {
    ...
    try {
      ...
      await apiFetch(`/admin/help/questions/${id}/promote`, {
        method: "POST",
  ...
  const dismissQuestion = async (id: string) => {
    try {
      await apiFetch(`/admin/help/questions/${id}`, {
        method: "DELETE",
      });
    } catch (err) {
  ...
  const saveFaq = async () => {
    ...
    try {
      if (editingFaq?.id) {
        await apiFetch(`/admin/help/faq/${editingFaq.id}`, {
          method: "PUT",
          body: JSON.stringify({
  ...
  const deleteFaq = async (id: string) => {
    ...
    try {
      await apiFetch(`/admin/help/faq/${id}`, {
        method: "DELETE",
      });
      fetchFaqs();
    } catch (err) {
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[rgba(240,244,250,0.5)]">
              Review bot questions and manage the FAQ knowledge base
            </p>
          </div>
          {tab === "faq" && (
            <button
            ...
            {pendingQuestions.length === 0 ? (
              <div className="rounded-2xl border border-white/[0.07] bg-[#0c111d] py-12 text-center">
                ...
                <p className="text-xs text-[rgba(240,244,250,0.4)] mt-1">
                  All user questions have been reviewed
                </p>
              </div>
            ) : (
              pendingQuestions.map((q) => {
                const form = promoteForm[q.id] ?? {
                ...
                return (
                  <div
                    key={q.id}
                    className="rounded-2xl border border-white/[0.07] bg-[#0c111d] p-5 space-y-4"
                  >
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <button
                        ...
                        className="shrink-0 text-xs text-[rgba(240,244,250,0.3)] hover:text-red-400 transition"
                      >
                        Dismiss
                      </button>
                    </div>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setPromoteForm((f) => ({
                              ...f,
                              [q.id]: { ...form, answer: e.target.value },
                            }))
                          }
                          className="input-field mt-1 resize-none text-sm"
                          placeholder="Write or edit the FAQ answer…"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setPromoteForm((f) => ({
                              ...f,
                              [q.id]: { ...form, category: e.target.value },
                            }))
                          }
                          className="input-field mt-1 text-sm"
                          placeholder="e.g. billing, agents, payouts"
                        />
                      </div>
                    </div>
                    <button
                      onClick={() => promoteQuestion(q.id)}
                      disabled={promoting === q.id || !form.answer.trim()}
        ...
        {tab === "faq" && (
          <div className="space-y-3">
            ...
            {showNewForm && (
              <div className="rounded-2xl border border-[#14b8a6]/30 bg-[#14b8a6]/5 p-5 space-y-3">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={saveFaq}
                    disabled={savingFaq || !newFaq.question || !newFaq.answer}
                    className="rounded-lg bg-[#14b8a6] px-4 py-2 text-xs font-semibold text-[#05080f] hover:bg-[#0d9488] transition disabled:opacity-40"
                  >
                  ...
                  <button
                    ...
                    className="rounded-lg border border-white/[0.08] px-4 py-2 text-xs text-[rgba(240,244,250,0.5)] hover:text-white transition"
                  >
                    Cancel
                  </button>
                </div>
            ...
            {faqs.length === 0 && !showNewForm ? (
              <div className="rounded-2xl border border-white/[0.07] bg-[#0c111d] py-12 text-center">
                <p className="text-sm text-[rgba(240,244,250,0.4)]">
                  No FAQ entries yet. Promote a bot question or add one manually.
                </p>
              </div>
            ) : (
              faqs.map((f) => (
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditingFaq((x) => (x ? { ...x, question: e.target.value } : null))
                          }
                          className="input-field mt-1"
                        />
                      </div>
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setEditingFaq((x) => (x ? { ...x, answer: e.target.value } : null))
                          }
                          className="input-field mt-1 resize-none"
                        />
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEditingFaq((x) => (x ? { ...x, category: e.target.value } : null))
                            }
                            className="input-field mt-1"
                          />
                        </div>
                        <div className="flex items-end pb-0.5">
                          <label className="flex items-center gap-2 cursor-pointer">
                            <div
                              onClick={() =>
                                setEditingFaq((x) => (x ? { ...x, published: !x.published } : null))
                              }
                              className={`relative h-5 w-9 rounded-full transition-colors ${editingFaq.published ? "bg-[#14b8a6]" : "bg-white/20"}`}
                            >
                              <span
                                className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${editingFaq.published ? "translate-x-4" : "translate-x-0.5"}`}
                              />
                            </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[10px] font-bold uppercase tracking-wider text-[rgba(240,244,250,0.35)]">
                            {f.category}
                          </span>
                          <span
                            className={`text-[10px] font-bold ${f.published ? "text-green-400" : "text-orange-400"}`}
                          >
                            ● {f.published ? "Live" : "Draft"}
                          </span>
                          <span className="text-[10px] text-[rgba(240,244,250,0.3)]">
                            👍 {f.helpful} · 👎 {f.notHelpful}
                          </span>
                        </div>
                        <p className="text-sm font-semibold text-[#f0f4fa] truncate">
                          {f.question}
                        </p>
                        <p className="mt-1 text-xs text-[rgba(240,244,250,0.5)] line-clamp-2">
                          {f.answer}
                        </p>
                      </div>
                      <div className="flex gap-2 shrink-0">
                        <button
                          ...
                          className="rounded-lg border border-white/[0.08] px-3 py-1.5 text-xs text-[rgba(240,244,250,0.5)] hover:text-white transition"
                        >
                          Edit
                        </button>
                        <button
