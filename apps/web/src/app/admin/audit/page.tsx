...
export default function GlobalAuditLogs() {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const { data } = await apiFetch<{ logs: any[]; total: number }>(
        `/admin/audit-logs?limit=${limit}&skip=${skip}`
      );
      if (data?.data) {
      ...
      <div className="p-8">
        <header className="mb-10">
          ...
          <p className="mt-2 text-[rgba(240,244,250,0.5)]">
            Monitor platform-wide tool executions and agent activity
          </p>
        </header>
                ...
                {loading && logs.length === 0 ? (
                  ...
                  logs.map((log) => (
                    <tr key={log.id} className="hover:bg-white/[0.02] transition">
                      ...
                      <td className="px-6 py-4 font-medium text-[#f0f4fa]">
                        {log.agent?.tenant?.name || "Unknown"}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${getStatusColor(log.status)}`}
                        >
                          {log.status}
                        </span>
                      </td>
