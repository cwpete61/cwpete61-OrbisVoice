...
import { API_BASE, User, apiFetch, authHeader, actionLoadingKey } from "@/lib/api";
...
function UsersContent() {
  ...
  const isSystemAdmin =
    profile?.role === "SYSTEM_ADMIN" || profile?.email === "myorbislocal@gmail.com";
  ...
  const fetchUsers = async (filter: "all" | "paid" | "free" = "all", searchTerm: string = "") => {
    ...
    try {
      ...
      const { data: resp } = await apiFetch<User[]>(`/admin/users?${params}`);
      setUsers(resp?.data || []);
    } catch {
    } finally {
  ...
  const fetchAffiliates = async (searchTerm: string = "") => {
    ...
    try {
      const params = new URLSearchParams({ filter: "affiliates" });
      if (searchTerm) params.set("search", searchTerm);
      const { data: resp } = await apiFetch<any[]>(`/admin/affiliates?${params}`);
  ...
  const handleUpdateAffiliateStatus = async (id: string, status: string) => {
    setActionLoading(actionLoadingKey("status", id));
    try {
      const { res } = await apiFetch(`/admin/affiliates/${id}/status`, {
        method: "POST",
  ...
  const handleSetCustomRate = async (aff: any) => {
    ...
    try {
      const { res, data } = await apiFetch(`/admin/affiliates/${aff.id}/commission-rate`, {
        method: "PATCH",
  ...
  const promoteToAffiliate = async (userId: string) => {
    ...
    try {
      const { res, data } = await apiFetch<{ message?: string }>("/admin/affiliates/promote", {
        method: "POST",
  ...
  const toggleBlockUser = async (user: User) => {
    ...
    try {
      ...
      if (res.ok) await fetchUsers(userFilter);
    } catch {
    } finally {
      setActionLoading(null);
    }
  ...
  const deleteUser = async (user: User) => {
    ...
    try {
      ...
      if (res.ok) await fetchUsers(userFilter, debouncedSearch);
    } catch {
    } finally {
      setActionLoading(null);
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${selectedUserIds.length} selected users? This cannot be undone.`))
      return;

    setActionLoading("bulk-delete");
    try {
      await Promise.all(
        selectedUserIds.map((id) => apiFetch(`/admin/users/${id}`, { method: "DELETE" }))
      );
      setSelectedUserIds([]);
      await fetchUsers(userFilter, debouncedSearch);
    } catch {
  ...
  const handleBulkBlock = async (isBlocked: boolean) => {
    ...
    if (
      !window.confirm(
        `${isBlocked ? "Block" : "Unblock"} ${selectedUserIds.length} selected users?`
      )
    )
      return;
                                  ...
                                  <label className="flex items-center gap-2 cursor-pointer ml-2">
                                    <input
                                      ...
                                      onChange={(e) =>
                                        setEditForm((prev) => ({
                                          ...prev,
                                          emailVerifiedByAdmin: e.target.checked,
                                        }))
                                      }
                                      className="w-4 h-4 rounded border-white/[0.1] bg-[#0c111d] text-[#14b8a6] focus:ring-offset-0 focus:ring-0"
                                    />
