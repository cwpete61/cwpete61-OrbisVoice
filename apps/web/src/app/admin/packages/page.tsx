...
export default function PackagesPage() {
  ...
  const fetchPackages = async () => {
    try {
      const { data } = await apiFetch<Package[]>("/admin/packages");
      if (data?.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const path = editingPackage ? `/admin/packages/${editingPackage.id}` : `/admin/packages`;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {packages.map((pkg) => (
              ...
              >
                ...
                <div className="mb-4">
                  <span
                    className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded ${pkg.active ? "bg-emerald-500/10 text-emerald-400" : "bg-red-500/10 text-red-400"}`}
                  >
                ...
                <div className="mt-4 flex items-center gap-2 text-slate-400">
                  ...
                  <span className="text-sm font-medium">
                    {pkg.credits.toLocaleString()} Conversations
                  </span>
                </div>
              </div>
            ))}
            ...
            <div className="bg-[#0f172a] border border-white/10 rounded-2xl w-full max-w-md overflow-hidden transform transition-all shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  >
                    Package is active for purchase
                  </label>
                </div>
                <button
                  type="submit"
