...
import { apiFetch, SubscriberDetail, TwilioConfig, TenantGoogleConfig } from "@/lib/api";
...
function SubscriberDetailContent({ id }: { id: string }) {
  ...
  const [tab, setTab] = useState<"overview" | "users" | "agents" | "settings" | "api-keys">(
    "overview"
  );
  ...
  const [tenantGoogleConfig, setTenantGoogleConfig] = useState<TenantGoogleConfig>({
    clientId: "",
    clientSecret: "",
    geminiApiKey: "",
    hasConfig: false,
  ...
  const fetchOverview = useCallback(async () => {
    try {
      const { data } = await apiFetch<SubscriberDetail>(`/admin/subscribers/${id}/overview`);
      if (data?.data) {
        setSub(data.data);
      }
    } catch (err) {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      const { data } = await apiFetch<{ twilio?: TwilioConfig; google?: TenantGoogleConfig }>(
        `/admin/subscribers/${id}/settings`
      );
      if (data?.data) {
        setTwilioConfig(
        ...
        setTenantGoogleConfig(
          data.data.google || { clientId: "", clientSecret: "", geminiApiKey: "", hasConfig: false }
        );
      }
    } catch (err) {
  ...
  const saveTenantSettings = async (type: "twilio" | "google") => {
    ...
    try {
      const { data } = await apiFetch<{ message?: string }>(`/admin/subscribers/${id}/settings`, {
        method: "POST",
  ...
  const deleteTenantSettings = async (type: "twilio" | "google") => {
    if (!confirm(`Are you sure you want to remove the ${type} configuration for this tenant?`))
      return;
    try {
      await apiFetch(`/admin/subscribers/${id}/settings/${type}`, {
        method: "DELETE",
      });
      fetchSettings();
    } catch (err) {
  ...
  const createApiKey = async () => {
    ...
    setCreatingKey(true);
    try {
      const { data } = await apiFetch<{ id: string; name: string; key: string; createdAt: string }>(
        `/admin/subscribers/${id}/api-keys`,
        {
          method: "POST",
  ...
  const revokeApiKey = async (keyId: string) => {
    ...
    try {
      await apiFetch(`/admin/subscribers/${id}/api-keys/${keyId}`, {
        method: "DELETE",
      });
  ...
  const liftHold = async (affiliateId: string) => {
    ...
    try {
      await apiFetch(`/admin/affiliates/${affiliateId}/lift-hold`, {
        method: "POST",
      });
    } catch (err) {
  ...
  if (!sub) {
    return (
      <DashboardShell tokenLoaded={tokenLoaded}>
        <div className="flex h-screen items-center justify-center text-sm text-[rgba(240,244,250,0.4)]">
    ...
    <DashboardShell tokenLoaded={tokenLoaded}>
      ...
      <div className="flex items-center gap-2 bg-blue-500/10 border-b border-blue-500/20 px-6 py-2">
        ...
        <p className="text-xs text-blue-300">
          <span className="font-bold">Admin Management View</span> — You are managing {sub.name}
          &apos;s account settings.
        </p>
      </div>
      ...
      <div className="px-6 lg:px-8 py-8 max-w-5xl mx-auto">
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <a
                href="/admin/tenants"
                className="text-xs text-[rgba(240,244,250,0.4)] hover:text-white transition"
              >
                ← Subscribers
              </a>
            </div>
        ...
        <div className="grid gap-4 sm:grid-cols-4 mb-8">
          {[
            ...
            <div
              key={s.label}
              className="rounded-2xl border border-white/[0.06] bg-[#0c111d] p-5 text-center"
            >
              <p className="text-xs font-semibold uppercase tracking-wider text-[rgba(240,244,250,0.35)]">
                {s.label}
              </p>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-white/[0.07] bg-[#0c111d] p-6 grid gap-3 sm:grid-cols-2 text-sm">
              {[
                ...
                [
                  "Subscription Ends",
                  sub.subscriptionEnds ? new Date(sub.subscriptionEnds).toLocaleDateString() : "—",
                ],
                [
                  "Usage Reset",
                  sub.usageResetAt ? new Date(sub.usageResetAt).toLocaleDateString() : "—",
                ],
                ["Joined", new Date(sub.createdAt).toLocaleDateString()],
              ].map(([k, v]) => (
                <div
                  key={k}
                  className="flex items-center justify-between gap-4 rounded-xl border border-white/[0.04] bg-white/[0.01] px-4 py-3"
                >
                  <span className="text-xs text-[rgba(240,244,250,0.4)]">{k}</span>
                  <span className="text-xs font-mono text-[#f0f4fa] truncate max-w-[60%] text-right">
                    {v}
                  </span>
                </div>
              ))}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/[0.05] text-left">
                  <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-[rgba(240,244,250,0.35)]">
                    Name
                  </th>
                  <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-[rgba(240,244,250,0.35)]">
                    Email
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {sub.users.map((u) => (
                  <tr key={u.id} className="hover:bg-white/[0.01]">
                    ...
                    <td className="px-5 py-3 text-xs text-[rgba(240,244,250,0.4)]">
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {sub.agents.map((a) => (
                  <tr key={a.id} className="hover:bg-white/[0.01]">
                    ...
                    <td className="px-5 py-3 text-[rgba(240,244,250,0.6)] font-mono text-xs">
                      {a.voiceId ?? "—"}
                    </td>
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-[#0c111d] p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs text-[rgba(240,244,250,0.45)] mt-1">
                    Configure SMS and voice capabilities for this workspace
                  </p>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-medium text-[rgba(240,244,250,0.6)] mb-1.5">
                    Account SID
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full rounded-lg border border-white/[0.08] bg-[#05080f] px-4 py-2 text-sm text-[#f0f4fa] outline-none focus:border-[#14b8a6]/50 transition"
                    placeholder="AC..."
                  />
                </div>
                <div>
            ...
            <div className="rounded-2xl border border-white/[0.07] bg-[#0c111d] p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs text-[rgba(240,244,250,0.45)] mt-1">
                    Tenant-specific OAuth and AI credentials
                  </p>
                </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.05] text-left">
                    ...
                    <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-[rgba(240,244,250,0.35)] text-right">
                      Action
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.03]">
                  {keysLoading ? (
                    <tr>
                      <td
                        colSpan={4}
                        className="px-5 py-12 text-center text-[rgba(240,244,250,0.4)]"
                      >
                        Loading keys...
                      </td>
                    </tr>
                    ...
                    apiKeys.map((k) => (
                      <tr key={k.id} className="hover:bg-white/[0.01]">
                        ...
                        <td className="px-5 py-4 text-xs text-[rgba(240,244,250,0.4)]">
                          {new Date(k.createdAt).toLocaleDateString()}
                        </td>
                        <td className="px-5 py-4 text-right">
                          <button
                            onClick={() => revokeApiKey(k.id)}
                            className="text-xs text-red-400/70 hover:text-red-400 transition underline underline-offset-4"
                          >
                            Revoke
                          </button>
                        </td>
        ...
        <div className="mt-8 rounded-2xl border border-white/[0.07] bg-[#0c111d] p-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {sub.users.some((u) => u.affiliate?.payoutHeld) && (
              <button
                onClick={() =>
                  sub.users.forEach((u) => u.affiliate?.payoutHeld && liftHold(u.affiliate.id))
                }
...
export default function AdminSubscriberDetailPage({ params }: { params: { id: string } }) {
  return (
    <Suspense>
      <SubscriberDetailContent id={params.id} />
    </Suspense>
  );
}
