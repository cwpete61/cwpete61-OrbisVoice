...
import { apiFetch, NotificationTemplate } from "@/lib/api";
...
function AdminNotificationsContent() {
  ...
  const fetchTemplates = useCallback(async () => {
    try {
      const { data } = await apiFetch<NotificationTemplate[]>("/admin/notifications/templates");
      if (data?.data) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setSendErr(null);
    try {
      const ids = sendForm.userIds
        .trim()
        .split(/[\n,]+/)
        .map((s) => s.trim())
        .filter(Boolean);
      const { data } = await apiFetch<{ sent?: number; message?: string }>(
      ...
      if (data?.data?.sent !== undefined) {
        setSendResult(`Sent to ${data.data.sent ?? "??"} users`);
      } else {
        setSendErr(data?.data?.message || "Failed to send");
      }
    } catch (err) {
      setSendErr("Failed to send");
    } finally {
      setSending(false);
    }
  };
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      await apiFetch(`/admin/notifications/templates/${editingTemplate.type}`, {
        method: "PUT",
        body: JSON.stringify({
      ...
      <div className="px-6 lg:px-8 py-8 max-w-5xl mx-auto">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-[rgba(240,244,250,0.5)]">
            Broadcast messages and manage email templates
          </p>
        </div>
            ...
            <form onSubmit={handleSend} className="space-y-4">
              <div>
                <label className="label-sm">
                  User IDs{" "}
                  <span className="text-[rgba(240,244,250,0.3)]">(leave blank for all users)</span>
                </label>
                <textarea
                  rows={3}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="flex items-end pb-0.5">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <div
                      onClick={() => setSendForm((f) => ({ ...f, sendEmail: !f.sendEmail }))}
                      className={`relative h-5 w-9 rounded-full transition-colors ${sendForm.sendEmail ? "bg-[#14b8a6]" : "bg-white/20"}`}
                    >
                      <span
                        className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${sendForm.sendEmail ? "translate-x-4" : "translate-x-0.5"}`}
                      />
                    </div>
          ...
          <div className="space-y-3">
            {templates.length === 0 ? (
              ...
              templates.map((tpl) => (
                <div
                  key={tpl.id}
                  className="rounded-2xl border border-white/[0.07] bg-[#0c111d] p-5"
                >
                  {editingTemplate?.id === tpl.id ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          <div
                            onClick={() =>
                              setEditingTemplate((t) => (t ? { ...t, enabled: !t.enabled } : null))
                            }
                            className={`relative h-5 w-9 rounded-full transition-colors ${editingTemplate.enabled ? "bg-[#14b8a6]" : "bg-white/20"}`}
                          >
                            <span
                              className={`absolute top-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${editingTemplate.enabled ? "translate-x-4" : "translate-x-0.5"}`}
                            />
                          </div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditingTemplate((t) =>
                              t ? { ...t, subject: e.target.value } : null
                            )
                          }
                          className="input-field mt-1"
                        />
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setEditingTemplate((t) =>
                              t ? { ...t, bodyHtml: e.target.value } : null
                            )
                          }
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-widest text-[rgba(240,244,250,0.4)]">
                          {tpl.type.replace(/_/g, " ")}
                        </p>
                        <p className="mt-1 text-sm font-medium text-[#f0f4fa]">{tpl.subject}</p>
                        <div className="mt-1 flex items-center gap-2">
                          <span
                            className={`text-[10px] font-bold ${tpl.enabled ? "text-green-400" : "text-red-400"}`}
                          >
