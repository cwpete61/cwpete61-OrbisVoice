...
async function main() {
    ...
    if (!admin.affiliate) {
        ...
        await affiliate_1.affiliateManager.applyForAffiliate(admin.id, "ACTIVE");
    }
    if (!user.affiliate) {
        console.log("Creating affiliate for user...");
        await affiliate_1.affiliateManager.applyForAffiliate(user.id, "ACTIVE");
    }
    ...
    if (!aff1 || !aff2)
        throw new Error("Failed to ensure affiliates exist");
    ...
    const results = await affiliate_1.affiliateManager.processBulkPayouts(ids);
