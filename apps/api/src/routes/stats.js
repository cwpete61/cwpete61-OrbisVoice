...
async function statsRoutes(fastify) {
    ...
    fastify.get("/stats/sales-chart", { onRequest: [auth_1.requireNotBlocked] }, async (request, reply) => {
        try {
            ...
            const transactions = await db_1.prisma.rewardTransaction.findMany({
                where: {
            ...
            transactions.forEach(t => {
                ...
                const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
                if (dataByMonth[key]) {
                    const isAffiliate = t.referrer?.affiliate !== null && t.referrer?.affiliate !== undefined;
            ...
            const organicUsers = await db_1.prisma.user.findMany({
                where: {
                    createdAt: { gte: sixMonthsAgo },
            ...
            organicUsers.forEach(u => {
                ...
                if (tier && tier !== 'free') {
                    ...
                    if (dataByMonth[key]) {
                        ...
                        if (tier === 'ltd')
                            val = 149;
                        if (tier === 'ai-revenue-infrastructure')
                            val = 299;
            ...
            logger_1.logger.error(err, "Failed to fetch sales chart data");
            return reply.code(500).send({
                ok: false,
                message: "Internal server error",
            });
    ...
    fastify.get("/stats/pricing-limits", async (request, reply) => {
        try {
            ...
            if (!settings) {
                settings = await db_1.prisma.platformSettings.findFirst();
            }
            const data = settings || {
                starterLimit: 1000,
            ...
            logger_1.logger.error(err, "Failed to fetch pricing limits");
            return reply.code(500).send({
                ok: false,
                message: "Internal server error",
            });
