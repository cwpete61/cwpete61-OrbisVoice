...
export async function adminRoutes(fastify: FastifyInstance) {
    ...
    fastify.patch("/admin/users/:id", async (request, reply) => {
        try {
            ...
            const { isAdmin, isBlocked, name, email, role, tier, commissionLevel } = request.body as {
                isAdmin?: boolean,
                isBlocked?: boolean,
                name?: string,
                email?: string,
            ...
            const user = await prisma.user.update({
                ...
                data: {
                    ...
                    ...(commissionLevel !== undefined && { commissionLevel: commissionLevel as any }),
                }
            });

            if (tier && existingUser.tenantId) {
