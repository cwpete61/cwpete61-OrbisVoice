...
export async function helpRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: { question: string } }>("/help/chat", async (request, reply) => {
    try {
  ...
  fastify.post<{ Params: { id: string }; Body: { answer: string; category?: string } }>(
    ...
    async (request, reply) => {
      try {
        const { id } = request.params;
        const { answer, category = "general" } = request.body as {
          answer: string;
          category?: string;
        };
        ...
        if (!q)
          return reply.code(404).send({ ok: false, message: "Question not found" } as ApiResponse);
        ...
        await prisma.faqQuestion.update({
          where: { id },
          data: { status: "answered", faqEntryId: entry.id },
        });
    ...
    async (request, reply) => {
      try {
        ...
        if (!question || !answer) {
          return reply
            .code(400)
            .send({ ok: false, message: "question and answer are required" } as ApiResponse);
        }

        const entry = await prisma.faqEntry.create({
          data: { question, answer, category, published, source: "admin" },
        });
        updateFaqEmbedding(entry.id).catch(() => {});
    ...
    async (request, reply) => {
      try {
        ...
        const entry = await prisma.faqEntry.update({
          where: { id },
          data: { question, answer, category, published },
        });
        updateFaqEmbedding(entry.id).catch(() => {});
        return reply.code(200).send({ ok: true, data: entry } as ApiResponse);
      } catch (err) {
