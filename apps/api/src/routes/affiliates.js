...
const PublicApplySchema = zod_1.z.object({
    email: zod_1.z.string().email().refine(email => email.toLowerCase().endsWith("@gmail.com"), {
...
async function affiliateRoutes(fastify) {
    ...
    fastify.get("/affiliates/program-details", async (request, reply) => {
        try {
            ...
            if (settings.defaultCommissionLevel === "MED")
                commissionRate = settings.medCommission;
            if (settings.defaultCommissionLevel === "HIGH")
                commissionRate = settings.highCommission;
            return reply.send({
                ok: true,
                data: {
                    commissionRate,
    ...
    fastify.post("/affiliates/public-apply", async (request, reply) => {
        try {
            ...
            const result = await affiliate_1.affiliateManager.applyForAffiliate(user.id, "ACTIVE");
            if (!result.success) {
            ...
            return reply.send({ ok: true, message: "Welcome to the OrbisVoice Partner Program!", data: { token } });
        }
        catch (err) {
    ...
    fastify.get("/affiliates/me/payouts", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            ...
            const payouts = await db_1.prisma.affiliatePayout.findMany({
                where: { affiliateId: affiliate.id },
                orderBy: { createdAt: "desc" },
            });
            return reply.send({ ok: true, data: payouts });
        }
        catch (err) {
            fastify.log.error(err);
            return reply.code(500).send({ ok: false, message: "Server error" });
        ...
        try {
            ...
            if (!affiliate) {
                ...
                if (!result.success || !result.data) {
                    return reply.code(500).send({ ok: false, message: "Failed to initialize partner profile" });
                }
    ...
    fastify.post("/affiliates/stripe/onboard", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            ...
            if (!affiliate) {
                ...
                affiliate = await db_1.prisma.affiliate.findUnique({
                    where: { userId },
                    include: { user: true }
                });
            }
            ...
            if (filter === "referrers") {
                ...
                const data = users.map((u) => {
                    ...
                    return {
                        ...
                        status: "ACTIVE", // All users are pre-approved referrers
        ...
        try {
            ...
            if (!affiliate) {
                ...
                if (user) {
                    ...
                    const result = await affiliate_1.affiliateManager.applyForAffiliate(user.id, status === "ACTIVE" ? "ACTIVE" : "PENDING");
                    if (!result.success || !result.data) {
                        return reply.code(400).send({ ok: false, message: result.message });
                    }
                    affiliate = await db_1.prisma.affiliate.findUnique({
    ...
    const CustomRateSchema = zod_1.z.object({
        customCommissionRate: zod_1.z.number().min(0).max(100).nullable(),
    });
    fastify.patch("/admin/affiliates/:id/commission-rate", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
            const { id } = request.params;
            const { customCommissionRate } = CustomRateSchema.parse(request.body);
            const affiliate = await db_1.prisma.affiliate.findUnique({ where: { id } });
            if (!affiliate) {
    ...
    const BulkPayoutSchema = zod_1.z.object({
        affiliateIds: zod_1.z.array(zod_1.z.string()).min(1),
    });
    fastify.post("/admin/affiliates/payout-bulk", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
            const { affiliateIds } = BulkPayoutSchema.parse(request.body);
            const results = await affiliate_1.affiliateManager.processBulkPayouts(affiliateIds);
            return reply.send({
                ok: true,
    ...
    fastify.get("/affiliates/stripe/account-details", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const affiliate = await db_1.prisma.affiliate.findUnique({ where: { userId } });
            if (!affiliate?.stripeAccountId) {
            ...
            let connectedBalance = { available: 0, pending: 0 };
            try {
    ...
    fastify.post("/affiliates/stripe/login-link", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const affiliate = await db_1.prisma.affiliate.findUnique({ where: { userId } });
            ...
            return reply.send({ ok: true, data: { url: loginLink.url } });
        }
        catch (err) {
            fastify.log.error(err);
    ...
    fastify.get("/affiliates/me/tax-status", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const affiliate = await db_1.prisma.affiliate.findUnique({ where: { userId } });
            ...
            const ytdTxs = await db_1.prisma.rewardTransaction.findMany({
                where: {
                    referrerId: userId,
            ...
            let taxForms = [];
            if (affiliate?.stripeAccountId && env_1.env.STRIPE_API_KEY) {
                try {
                    const stripe = new stripe_1.default(env_1.env.STRIPE_API_KEY, { apiVersion: "2024-06-20" });
                    // List tax forms for this connected account (Stripe Tax Forms API)
                    const forms = await stripe.tax?.forms?.list?.({ limit: 10 }, { stripeAccount: affiliate.stripeAccountId });
                    taxForms = forms?.data ?? [];
                }
                catch (_) {
    ...
    fastify.get("/affiliates/me/payout-eligibility", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            ...
            const ytdPaid = affiliate.payouts.reduce((sum, p) => sum + p.netAmount, 0);
    ...
    fastify.post("/affiliates/me/submit-tax-docs", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const { taxFormUrl, tinSsn } = request.body;
            ...
            const user = await db_1.prisma.user.findUnique({ where: { id: userId }, select: { taxFormUrl: true, tinSsn: true } });
    ...
    fastify.post("/admin/affiliates/:id/lift-hold", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
            const { id } = request.params;
            const adminId = request.user.userId;
            const affiliate = await db_1.prisma.affiliate.update({
                where: { id },
                data: {
                    payoutHeld: false,
            ...
            try {
                const { createNotification, NotifType } = await Promise.resolve().then(() => __importStar(require("../services/notification")));
