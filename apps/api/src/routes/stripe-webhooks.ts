      ...
      switch (event.type) {
        ...
        case "invoice.payment_succeeded": {
          ...
          if (billingReason === "subscription_cycle" || billingReason === "subscription_update") {
            logger.info(
              { invoiceId, billingReason, amount: amountPaid },
              "Skipping commission for recurring subscription payment"
            );
            break;
          }
          ...
          const sourceId = paymentIntentId || invoiceId;
          const processed = await referralManager.processCommission(user.id, amountPaid, sourceId);

          if (processed) {
            logger.info(
              { userId: user.id, amount: amountPaid, invoiceId },
              "Commission created from Stripe payment"
            );
          }
          break;
        }
        ...
        case "charge.refunded": {
          ...
          if (invoiceId || paymentIntentId) {
            const sourceIds = [invoiceId, paymentIntentId].filter(Boolean);
            const updated = await prisma.rewardTransaction.updateMany({
              where: {
                sourcePaymentId: { in: sourceIds as string[] },
        ...
        case "checkout.session.completed": {
          ...
          const isPackagePurchase = session.metadata?.type === "package";
          ...
          // Process commission for one-time paid checkouts only.
          // Subscriptions are handled by invoice.payment_succeeded to avoid double counting.
          if (!isPackagePurchase && !isSubscriptionCheckout) {
            const sourceId = paymentIntentId || sessionId;
            await referralManager.processCommission(user.id, amountTotal, sourceId);
          }
          ...
          if (isPackagePurchase) {
            ...
            if (creditsToAdd > 0) {
              ...
              logger.info(
                { tenantId, packageId, creditsToAdd },
                "Bonus credits added via package purchase"
              );
          ...
          if (
            ...
            env.STRIPE_API_KEY &&
            ltdHostingPrice &&
            !ltdHostingPrice.includes("placeholder")
          ) {
            try {
        ...
        case "customer.subscription.updated": {
          ...
          if (tier) {
            await UsageService.updateSubscriptionTier(tenant.id, tier, subscription.id);
          } else {
          ...
          logger.info(
            { tenantId: tenant.id, status, tier, subId: subscription.id },
            "Tenant subscription updated via webhook"
          );
          break;
        }
        ...
        case "customer.subscription.deleted": {
            ...
            logger.info(
              {
                ...
                currentSubId: tenant.stripeSubscriptionId,
              },
              "Ignoring deletion event for non-active subscription"
            );
            break;
          }
          ...
          logger.info(
            { tenantId: tenant.id, subId: subscription.id },
            "Tenant subscription deleted/canceled"
          );
          break;
        }
        ...
        case "transfer.created": {
          ...
          if (updated.count > 0) {
            logger.info(
              ...
              "Backfilled Stripe transfer ID on payout record"
            );
          }
          break;
        }
        ...
        case "account.updated": {
          ...
          if (newStatus !== affiliate.stripeAccountStatus) {
            ...
            logger.info(
              { affiliateId: affiliate.id, newStatus },
              "Stripe account status synced via webhook"
            );
          }
          break;
        }
