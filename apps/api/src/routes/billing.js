...
async function billingRoutes(fastify) {
    ...
    fastify.post("/billing/portal", { preHandler: [auth_1.authenticate] }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        if (!env_1.env.STRIPE_API_KEY) {
            return reply.code(500).send({ error: "Stripe API key not configured" });
        }
        try {
            ...
            if (!portalRes.ok) {
                ...
                logger_1.logger.error({ err: errText }, "Failed to create Stripe Portal session");
            ...
            logger_1.logger.error({ err }, "Portal creation error");
            return reply.code(500).send({ error: "Internal server error" });
        }
    });
    ...
    fastify.post("/billing/purchase-package", { preHandler: [auth_1.authenticate] }, async (request, reply) => {
        const tenantId = request.user.tenantId;
        ...
        try {
            const pkg = await db_1.prisma.conversationPackage.findUnique({
            ...
            logger_1.logger.info({ tenantId, packageId, credits: pkg.credits }, "Credits purchased");
            return reply.send({
                ok: true,
            ...
            logger_1.logger.error({ err }, "Failed to purchase package");
            return reply.code(500).send({ error: "Internal server error" });
        }
    });
}
