...
export async function affiliateRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/affiliates/me", { onRequest: [authenticate] }, async (request, reply) => {
    try {
      ...
      try {
        ...
        if (user) {
          if (!body.password) {
            return reply
              .code(400)
              .send({ ok: false, message: "Please provide your password to verify your account" });
          }
          ...
          if (!body.password) {
            return reply
              .code(400)
              .send({ ok: false, message: "Password is required for new accounts" });
          }
          // Create new user
          const hashedPassword = await bcrypt.hash(body.password, 10);
          const tenant = await prisma.tenant.create({
            data: { name: `${body.firstName}'s Workspace` },
          });
        ...
        return reply.send({
          ...
          message: verificationRequired
            ? "Application received! Please verify your email."
            : "Welcome to the OrbisVoice Partner Program!",
        ...
        if (err instanceof z.ZodError) {
          return reply
            .code(400)
            .send({ ok: false, message: "Invalid application data: " + err.errors[0].message });
        }
        fastify.log.error(err);
        return reply.code(500).send({ ok: false, message: "Server error" });
      }
  ...
  fastify.get("/affiliates/me/payouts", { onRequest: [authenticate] }, async (request, reply) => {
    try {
      const userId = (request.user as AuthPayload).userId;
      ...
      try {
        ...
        if (!affiliate) {
          ...
          if (!result.success || !result.data) {
            return reply
              .code(500)
              .send({ ok: false, message: "Failed to initialize partner profile" } as ApiResponse);
          }
    ...
    async (request, reply) => {
      try {
        if (!env.STRIPE_API_KEY) {
          return reply
            .code(400)
            .send({ ok: false, message: "Stripe integration is not configured" });
        }
        ...
        if (affiliate.stripeAccountStatus === "active") {
          return reply
            .code(400)
            .send({ ok: false, message: "Stripe account is already connected and active." });
        }
        ...
        if (env.STRIPE_API_KEY) {
          ...
          return reply
            .code(400)
            .send({ ok: false, message: "Stripe integration is not fully configured" });
        }
      } catch (err: any) {
  ...
  fastify.post("/affiliates/apply", { onRequest: [authenticate] }, async (request, reply) => {
    try {
      const userId = (request.user as AuthPayload).userId;
  ...
  fastify.get("/admin/affiliates", { onRequest: [requireAdmin] }, async (request, reply) => {
    try {
      ...
      try {
        ...
        if (!affiliate) {
          ...
          if (user) {
            ...
            const result = await affiliateManager.applyForAffiliate(
              user.id,
        ...
        if (status === "ACTIVE") {
          ...
          if (!affiliate.lockedCommissionRate) {
            const settings = await prisma.platformSettings.findUnique({ where: { id: "global" } });
            const userLevel =
              affiliate.user?.commissionLevel || settings?.defaultCommissionLevel || "LOW";
    ...
    async (request, reply) => {
      try {
        ...
        if (err instanceof z.ZodError) {
          return reply
            .code(400)
            .send({ ok: false, message: "Invalid request data: " + err.errors[0].message });
        }
        fastify.log.error(err);
        return reply.code(500).send({ ok: false, message: "Server error" });
      }
    ...
    async (request, reply) => {
      try {
        ...
        const user = await prisma.user.findUnique({
          where: { id: userId },
          select: { taxFormUrl: true, tinSsn: true },
        });
