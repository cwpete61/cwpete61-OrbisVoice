...
const AdminUpdateUserSchema = zod_1.z.object({
    ...
    role: zod_1.z.enum(["SYSTEM_ADMIN", "ADMIN", "USER"]).optional(),
...
const PlatformSettingsSchema = zod_1.z.object({
    ...
    transactionFeePercent: zod_1.z.number().min(0).max(100).default(3.4),
...
async function userRoutes(fastify) {
    ...
    fastify.get("/users/me", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
    ...
    // Update user profile
    fastify.put("/users/me", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
    ...
    // Update password
    fastify.put("/users/me/password", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
    ...
    // Upload avatar
    fastify.post("/users/me/avatar", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
    ...
    // Admin: list users
    fastify.get("/admin/users", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            const subFilter = query.subFilter;
    ...
    // Admin: create user
    fastify.post("/admin/users", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
    ...
    fastify.get("/admin/users/:id", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
    ...
    // Admin: update user
    fastify.put("/admin/users/:id", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
            ...
            if (body.role !== undefined || body.isAdmin !== undefined) {
                ...
                const dbRequestingUser = await db_1.prisma.user.findUnique({
            ...
            if (Object.keys(userData).length > 0) {
                operations.push(db_1.prisma.user.update({
                    where: { id: targetId },
                    data: userData,
                    select: userSelect,
                }));
                ...
                operations.push(db_1.prisma.user.findUnique({
                    where: { id: targetId },
                    select: userSelect,
                }));
            }
            if (tier) {
                operations.push(db_1.prisma.tenant.update({
            ...
            const [user] = await db_1.prisma.$transaction(operations);
            return reply.send({
                ok: true,
                data: user,
    ...
    fastify.put("/admin/users/:id/block", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const targetId = request.params.id;
    ...
    fastify.put("/admin/users/:id/password", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const targetId = request.params.id;
    ...
    fastify.post("/admin/users/:id/impersonate", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const targetId = request.params.id;
            const user = await db_1.prisma.user.findUnique({
                where: { id: targetId },
    ...
    // Admin: delete user
    fastify.delete("/admin/users/:id", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const targetId = request.params.id;
            const targetUser = await db_1.prisma.user.findUnique({
                where: { id: targetId },
    ...
    fastify.get("/admin/google-auth/config", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const config = await db_1.prisma.googleAuthConfig.findUnique({
    ...
    fastify.put("/admin/google-auth/config", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const body = request.body || {};
            const config = await db_1.prisma.googleAuthConfig.upsert({
                where: { id: "google-auth-config" },
                update: {
    ...
    fastify.get("/admin/system-email", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const config = await db_1.prisma.systemEmailConfig.findUnique({
                where: { id: "global" },
            });
            return reply.send({
                ok: true,
    ...
    fastify.put("/admin/system-email", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const body = request.body || {};
            const config = await db_1.prisma.systemEmailConfig.upsert({
                where: { id: "global" },
                update: {
                    username: body.username,
    ...
    fastify.get("/admin/stripe-connect", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const config = await (db_1.prisma).stripeConnectConfig.findUnique({
                where: { id: "global" },
            });
            return reply.send({
                ok: true,
    ...
    fastify.put("/admin/stripe-connect", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const body = request.body || {};
            const config = await (db_1.prisma).stripeConnectConfig.upsert({
                where: { id: "global" },
                update: {
                    clientId: body.clientId,
    ...
    fastify.post("/admin/stripe-connect/test", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
    ...
    fastify.post("/admin/system-email/test", { onRequest: [auth_1.requireAdmin] }, async (request, reply) => {
        try {
    ...
    fastify.get("/users/me/calendar", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            const creds = await db_1.prisma.calendarCredentials.findUnique({
                where: {
                    userId_tenantId: {
                        userId,
                        tenantId,
    ...
    fastify.delete("/users/me/calendar", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            await db_1.prisma.calendarCredentials.deleteMany({
                where: {
                    userId,
                    tenantId,
                },
    ...
    fastify.get("/users/me/gmail", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            const creds = await db_1.prisma.gmailCredentials.findUnique({
                where: {
                    userId_tenantId: {
                        userId,
                        tenantId,
    ...
    fastify.delete("/users/me/gmail", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            await db_1.prisma.gmailCredentials.deleteMany({
                where: {
                    userId,
                    tenantId,
                },
    ...
    fastify.get("/users/me/gmail/connect-url", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
    ...
    fastify.post("/users/me/gmail/connect", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            const body = request.body;
            ...
            await db_1.prisma.gmailCredentials.upsert({
                where: {
                    userId_tenantId: {
                        userId,
                        tenantId,
    ...
    fastify.get("/users/me/gmail/credentials", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            const creds = await db_1.prisma.gmailCredentials.findUnique({
    ...
    // Disconnect Gmail
    fastify.delete("/users/me/gmail/disconnect", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            await db_1.prisma.gmailCredentials.delete({
                where: {
                    userId_tenantId: {
                        userId,
                        tenantId,
    ...
    fastify.post("/users/me/gmail/verify", { onRequest: [auth_1.authenticate] }, async (request, reply) => {
        try {
            const userId = request.user.userId;
            const tenantId = request.user?.tenantId;
            const creds = await db_1.prisma.gmailCredentials.findUnique({
            ...
            try {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        await db_1.prisma.gmailCredentials.update({
                            where: { id: creds.id },
                            data: { verified: false },
                        });
                        return reply.code(401).send({
    ...
    fastify.get("/admin/platform-settings", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            let settings = await db_1.prisma.platformSettings.findUnique({
                where: { id: "global" },
            });
            if (!settings) {
    ...
    fastify.put("/admin/platform-settings", { onRequest: [auth_1.requireSystemAdmin] }, async (request, reply) => {
        try {
            const body = PlatformSettingsSchema.parse(request.body);
            const oldSettings = await db_1.prisma.platformSettings.findUnique({
                where: { id: "global" },
                select: { defaultCommissionLevel: true }
            });
            const oldLevel = oldSettings?.defaultCommissionLevel || "LOW";
            const settings = await db_1.prisma.platformSettings.upsert({
                where: { id: "global" },
                update: body,
                create: {
                    id: "global",
