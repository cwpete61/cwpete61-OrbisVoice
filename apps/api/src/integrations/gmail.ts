...
class GmailClient {
   ...
   */
  async refreshToken(refreshToken: string): Promise<GmailCredentials> {
    try {
      const response = await fetch("https://oauth2.googleapis.com/token", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
   ...
   */
  async sendEmail(accessToken: string, message: EmailMessage): Promise<string> {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error({ status: response.status, error }, "Gmail send failed");
      ...
      logger.error({ err, to: message.to }, "Failed to send email via Gmail");
      throw err;
    }
  }
   ...
   */
  async getMessages(accessToken: string, query?: string, maxResults: number = 10): Promise<any[]> {
    try {
