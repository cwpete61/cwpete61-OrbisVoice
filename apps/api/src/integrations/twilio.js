    ...
    async sendSms(message) {
        try {
            ...
            if (!response.ok) {
                ...
                logger_1.logger.error({ status: response.status, error }, "Twilio SMS send failed");
                throw new Error(`Twilio error: ${response.status}`);
            }
            const data = await response.json();
            logger_1.logger.info({ messageSid: data.sid, to: message.to }, "SMS sent successfully");
            return data.sid;
        }
        catch (err) {
            logger_1.logger.error({ err, to: message.to }, "Failed to send SMS via Twilio");
            throw err;
        }
    }
    /**
...
<Response>
  ...
  ${config.record ? "<Record/>" : ""}
            ...
            if (!response.ok) {
                const error = await response.text();
                logger_1.logger.error({ status: response.status, error }, "Twilio call initiation failed");
                throw new Error(`Twilio error: ${response.status}`);
            }
            const data = await response.json();
            logger_1.logger.info({ callSid: data.sid, to: config.to }, "Call initiated");
            return data.sid;
        }
        catch (err) {
            logger_1.logger.error({ err, to: config.to }, "Failed to initiate Twilio call");
            throw err;
        }
    }
    /**
    ...
    async getCall(callSid) {
        try {
            ...
            logger_1.logger.error({ err, callSid }, "Failed to get call information");
            throw err;
        }
    }
    /**
    ...
    async getPhoneNumbers() {
        try {
            ...
            logger_1.logger.error({ err }, "Failed to get Twilio phone numbers");
            throw err;
        }
    }
}
