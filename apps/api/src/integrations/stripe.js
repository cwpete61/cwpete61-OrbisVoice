...
class StripeClient {
    ...
    async createTransfer(transfer) {
        try {
            ...
            if (!response.ok) {
                const error = await response.text();
                logger_1.logger.error({ status: response.status, error }, "Stripe transfer creation failed");
                throw new Error(`Stripe error: ${response.status} - ${error}`);
            }
            const data = (await response.json());
            logger_1.logger.info({ transferId: data.id, destination: transfer.destination, amount: transfer.amount }, "Transfer created");
            return data;
        }
        catch (err) {
            logger_1.logger.error({ err }, "Failed to create Stripe transfer");
            throw err;
        }
    }
}
