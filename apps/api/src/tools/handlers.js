...
function getGmailClient() {
    if (!gmailClient) {
        gmailClient = new gmail_1.GmailClient({
            clientId: env_1.env.GMAIL_CLIENT_ID || "",
            clientSecret: env_1.env.GMAIL_CLIENT_SECRET || "",
            redirectUri: env_1.env.GMAIL_REDIRECT_URI || "http://localhost:3000/api/oauth/gmail/callback",
        });
    }
    return gmailClient;
}
function getCalendarClient() {
    if (!calendarClient) {
        calendarClient = new calendar_1.GoogleCalendarClient({
            clientId: env_1.env.GOOGLE_CALENDAR_CLIENT_ID || "",
            clientSecret: env_1.env.GOOGLE_CALENDAR_CLIENT_SECRET || "",
            redirectUri: env_1.env.GOOGLE_CALENDAR_REDIRECT_URI || "http://localhost:3000/api/oauth/calendar/callback",
        });
    }
...
function getStripeClient() {
    if (!stripeClient) {
        stripeClient = new stripe_1.StripeClient({
...
function getTwilioClient() {
    if (!twilioClient) {
        twilioClient = new twilio_1.TwilioClient({
            accountSid: env_1.env.TWILIO_ACCOUNT_SID || "",
            authToken: env_1.env.TWILIO_AUTH_TOKEN || "",
            phoneName: env_1.env.TWILIO_PHONE_NUMBER || "",
        });
    }
    return twilioClient;
}
...
const escalateToHumanHandler = async (input, _context) => {
...
const sendMessageHandler = async (input, context) => {
    try {
        ...
        if (channel === "email") {
            try {
                ...
                if (!accessToken) {
                    logger_1.logger.warn("No Gmail access token available");
                    return {
                        success: true,
                        data: {
                ...
                logger_1.logger.error({ err }, "Failed to send email");
                return {
                    success: false,
        ...
        logger_1.logger.error({ err }, "Failed to send message");
        return {
            success: false,
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const scoreLeadHandler = async (input, _context) => {
...
// Product Agent Tools
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const getProductInfoHandler = async (input, _context) => {
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const checkInventoryHandler = async (input, _context) => {
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const checkAvailabilityHandler = async (input, _context) => {
    try {
        const calendar = getCalendarClient();
        const accessToken = input.accessToken || process.env.GOOGLE_CALENDAR_TEST_TOKEN;
        if (!accessToken) {
            logger_1.logger.warn("No Google Calendar access token available");
            return {
                success: true,
                data: {
                    date: input.date,
        ...
        logger_1.logger.error({ err }, "Failed to check calendar availability");
        return {
            success: false,
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const createEventHandler = async (input, _context) => {
    try {
        const calendar = getCalendarClient();
        const accessToken = input.accessToken || process.env.GOOGLE_CALENDAR_TEST_TOKEN;
        if (!accessToken) {
            logger_1.logger.warn("No Google Calendar access token - event not created");
            return {
                success: true,
                data: {
        ...
        logger_1.logger.error({ err }, "Failed to create calendar event");
        return {
            success: false,
...
// Gmail Agent Tools
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const sendEmailHandler = async (input, _context) => {
    try {
        const gmail = getGmailClient();
        const accessToken = input.accessToken || process.env.GMAIL_TEST_TOKEN;
        if (!accessToken) {
            logger_1.logger.warn("No Gmail access token - email not sent");
            return {
                success: true,
                data: {
...
// Stripe Agent Tools
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const createPaymentHandler = async (input, _context) => {
    try {
        const stripe = getStripeClient();
        const apiKey = env_1.env.STRIPE_API_KEY;
        if (!apiKey) {
        ...
        const charge = await stripe.createCharge(input.customerId || "temp_customer", input.amount, input.currency || "usd", input.description);
        return {
            success: true,
            data: {
                chargeId: charge.id,
        ...
        logger_1.logger.error({ err }, "Failed to create charge");
        return {
            success: false,
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const getSubscriptionHandler = async (input, _context) => {
    try {
        const stripe = getStripeClient();
        const apiKey = env_1.env.STRIPE_API_KEY;
        if (!apiKey) {
        ...
        logger_1.logger.error({ err }, "Failed to get subscription");
        return {
            success: false,
...
// Twilio Agent Tools
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const sendSmsHandler = async (input, _context) => {
    try {
        ...
        logger_1.logger.error({ err }, "Failed to send SMS");
        return {
            success: false,
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const initiateCallHandler = async (input, _context) => {
    try {
        const twilio = getTwilioClient();
        const accountSid = env_1.env.TWILIO_ACCOUNT_SID;
        if (!accountSid) {
        ...
        logger_1.logger.error({ err }, "Failed to initiate call");
        return {
            success: false,
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const generateToneHandler = async (input, _context) => {
...
function registerToolHandlers() {
    // Communication Agent
    executor_1.toolExecutor.register("escalate_to_human", escalateToHumanHandler);
    executor_1.toolExecutor.register("send_message", sendMessageHandler);
    // Lead Qualification Agent
    executor_1.toolExecutor.register("score_lead", scoreLeadHandler);
    // Product Agent
    executor_1.toolExecutor.register("get_product_info", getProductInfoHandler);
    executor_1.toolExecutor.register("check_inventory", checkInventoryHandler);
    // Google Calendar Agent
    executor_1.toolExecutor.register("check_availability", checkAvailabilityHandler);
    executor_1.toolExecutor.register("create_event", createEventHandler);
    // Gmail Agent
    executor_1.toolExecutor.register("send_email", sendEmailHandler);
    // Stripe Agent
    executor_1.toolExecutor.register("create_payment", createPaymentHandler);
    executor_1.toolExecutor.register("get_subscription", getSubscriptionHandler);
    // Twilio Agent
    executor_1.toolExecutor.register("send_sms", sendSmsHandler);
    executor_1.toolExecutor.register("initiate_call", initiateCallHandler);
    // Phone Tones Agent
    executor_1.toolExecutor.register("generate_tone", generateToneHandler);
}
