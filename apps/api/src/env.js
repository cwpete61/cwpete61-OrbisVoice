...
const envSchema = zod_1.z.object({
    ...
    // Email System
    EMAIL_PROVIDER: zod_1.z.enum(["console", "sendgrid", "resend"]).default("console"),
    SENDGRID_API_KEY: zod_1.z.string().optional(),
    RESEND_API_KEY: zod_1.z.string().optional(),
    EMAIL_FROM: zod_1.z.string().default("noreply@orbisvoice.app"),
    EMAIL_FROM_NAME: zod_1.z.string().default("OrbisVoice"),
...
const parsed = result.success ? result.data : process.env;
// Log warnings for missing critical keys in production
if (parsed.NODE_ENV === "production") {
    const missingKeys = [];
    if (!parsed.STRIPE_API_KEY)
        missingKeys.push("STRIPE_API_KEY");
...
const corsOrigins = typeof corsOriginsStr === 'string'
    ? corsOriginsStr.split(',').map((o) => o.trim())
    : corsOriginsStr;
exports.env = {
    ...parsed,
