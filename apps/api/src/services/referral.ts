...
class ReferralManager {
  ...
  async createReferral(referrerId: string, rewardAmount?: number): Promise<string> {
    try {
      ...
      logger.error({ err, referrerId }, "Failed to create referral");
      throw err;
    }
  }
  ...
  async completeReferral(code: string): Promise<boolean> {
    try {
      ...
      logger.info({ code }, "Referral completed");
      return true;
    } catch (err) {
      logger.error({ err, code }, "Failed to complete referral");
      return false;
    }
  }
