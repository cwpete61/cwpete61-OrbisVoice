...
export class AffiliateManager {
  ...
  async applyForAffiliate(userId: string, status: "PENDING" | "ACTIVE" = "PENDING") {
    try {
      ...
      const baseSlug = (user.username || user.name.split(" ")[0] || "user")
        .toLowerCase()
        .replace(/[^a-z0-9]/g, "");
      let slug = baseSlug;
      let counter = 1;
      ...
      const userLevel =
        (user as { commissionLevel?: string }).commissionLevel ||
        settings?.defaultCommissionLevel ||
        "LOW";
  ...
  async getStats(userId: string) {
    try {
      ...
      return {
        ...affiliate,
        shareUrl: `${env.WEB_URL}/a/${affiliate.slug}`,
      ...
      const queue = await Promise.all(
        affiliates.map(async (aff: any) => {
          ...
          const complianceBlocked =
            aff.payoutHeld || (requiresTaxForm && (!aff.taxFormCompleted || !aff.tax1099Uploaded));
          ...
          return {
            ...
            requiresTaxForm,
            complianceBlocked,
            holdReason: aff.payoutHoldReason,
  ...
  async processPayout(affiliateId: string) {
    try {
      ...
      if (affiliate.payoutHeld) {
        throw new Error(affiliate.payoutHoldReason || "Payout is currently on hold");
      }

      const now = new Date();
      const startOfYear = new Date(now.getFullYear(), 0, 1);
      const nextYearStart = new Date(now.getFullYear() + 1, 0, 1);
      const ytdPaidAggregate = await prisma.affiliatePayout.aggregate({
        where: {
          affiliateId: affiliate.id,
          status: "PAID",
          createdAt: {
      ...
      const ytdPaid = ytdPaidAggregate._sum.amount || 0;
      const projectedYtdPaid = ytdPaid + payoutAmount;
      const needsTaxDocs = projectedYtdPaid >= 600;
      const taxComplete = affiliate.taxFormCompleted && affiliate.tax1099Uploaded;
      if (needsTaxDocs && !taxComplete) {
        throw new Error("Tax documentation required before payout (W-9 and tax form)");
      }

      if (payoutAmount < payoutMinimum) {
        throw new Error(
          `Earnings must be at least $${payoutMinimum} for payout. Current balance: $${payoutAmount.toFixed(2)}`
        );
      }
